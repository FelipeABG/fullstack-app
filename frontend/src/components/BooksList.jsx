export function BooksList({ books, setBooks, setBookEdit }) {
   const deleteBook = (id) => {
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-[20px]">
         ...
         <div className="w-[30vw] h-[80vh] rounded-lg border-2 border-[#dec8ac] overflow-scroll">
            ...
            {books.map((book) => (
               <div
                  key={book.id}
                  className="grid grid-cols-4 w-full h-[7vh] hover:bg-[#dec8ac] border-b border-[#dec8ac] items-center text-[#273b53] bg-[#fefaf3] cursor-default"
                  onClick={() => setBookEdit(book)}
               >
